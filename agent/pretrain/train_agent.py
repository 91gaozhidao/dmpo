...
from util.dirs import REINFLOW_DATA_DIR, REINFLOW_LOG_DIR
from env.gym_utils import make_async


def find_latest_checkpoint(name, log_base_dir=None):
    """
    Find the latest checkpoint for auto-resume.

    Args:
        name: The experiment name (e.g., 'kitchen-mixed-v0_pre_improved_meanflow_dispersive_mlp_ta4_infnce_d0p5_t0p5')
        log_base_dir: Base directory for logs (e.g., '/path/to/log/gym/pretrain')

    Returns:
        tuple: (checkpoint_path, target_epochs) or (None, None) if no checkpoint found
    """
    if log_base_dir is None:
        return None, None

    exp_dir = os.path.join(log_base_dir, name)
    ...
    for d in os.listdir(exp_dir):
        full_path = os.path.join(exp_dir, d)
        if os.path.isdir(full_path):
            checkpoint_path = os.path.join(full_path, "checkpoint", "last.pt")
            if os.path.exists(checkpoint_path):
                # Get modification time for sorting
                mtime = os.path.getmtime(checkpoint_path)
                timestamp_dirs.append((mtime, checkpoint_path, full_path))
    ...
    _, latest_checkpoint, latest_dir = timestamp_dirs[0]

    # Try to get target epochs from the config
    config_path = os.path.join(latest_dir, ".hydra", "config.yaml")
    target_epochs = None
    if os.path.exists(config_path):
        try:
            with open(config_path, 'r') as f:
                ...
                if config and 'train' in config and 'n_epochs' in config['train']:
                    target_epochs = config['train']['n_epochs']
        except Exception as e:
            log.warning(f"Failed to read config from {config_path}: {e}")

    return latest_checkpoint, target_epochs
...
class PreTrainAgent:
    def __init__(self, cfg):
        ...
        self.epoch = 1
        self.first_epoch = 0
        self.target_epochs = self.n_epochs  # The target total epochs to train

        # Auto-resume: automatically find and load the latest checkpoint
        self.auto_resume = cfg.get('auto_resume', False)
        self.resume_path = cfg.get('base_policy_path', None)

        if self.auto_resume and self.resume_path is None:
            # Try to find the latest checkpoint for this experiment
            exp_name = cfg.get('name', None)
            if exp_name:
                # Determine the log base directory from the logdir pattern
                # logdir format: ${REINFLOW_LOG_DIR}/gym/pretrain/${name}/${timestamp}_${seed}
                # We need: ${REINFLOW_LOG_DIR}/gym/pretrain/
                log_base_dir = os.path.dirname(os.path.dirname(self.logdir))

                auto_checkpoint, prev_target_epochs = find_latest_checkpoint(exp_name, log_base_dir)
                if auto_checkpoint:
                    self.resume_path = auto_checkpoint
                    log.info(f"[Auto-Resume] Found checkpoint: {auto_checkpoint}")
                    # Use the original target epochs if available
                    if prev_target_epochs is not None:
                        self.target_epochs = prev_target_epochs
                        log.info(f"[Auto-Resume] Using target epochs from previous run: {self.target_epochs}")
                else:
                    log.info(f"[Auto-Resume] No previous checkpoint found for '{exp_name}', starting fresh training.")
